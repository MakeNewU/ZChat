...
- (void)onReceiveCustomSystemNotification:(NIMCustomSystemNotification *)notification
{
    ...
    if (data) {
        ...
        {
//            [self refreshSessionTitle:@"对方正在输入"];
            [self refreshSessionSubTitle:@"对方正在输入"];//wzteng同时修改刷新方法onNTESTimerFired()
...
//    [self refreshSessionTitle:self.sessionTitle];
    [self refreshSessionSubTitle:self.sessionSubTitle];//wzteng
}
...
- (void)enterHistory:(id)sender{
    ...
    UIActionSheet *sheet = [[UIActionSheet alloc] initWithTitle:@"选择操作" delegate:nil cancelButtonTitle:@"取消" destructiveButtonTitle:nil otherButtonTitles:@"查看云消息记录",@"搜索本地消息记录",@"清空本地消息记录", nil];
    [sheet showInView:self.view completionHandler:^(NSInteger index) {
        switch (index) {
            case 0:{ //查看云端消息
